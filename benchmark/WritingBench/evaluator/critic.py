...
class CriticAgent(object):
    def __init__(self,
        ...
        self.model = LLM(
            ...
            tensor_parallel_size=4, # Your tensor parallel size setting. Defaults to 1, indicating no parallelism
        )
        self.tokenizer = AutoTokenizer.from_pretrained('AQuarterMile/WritingBench-Critic-Model-Qwen-7B')
        assert self.model.llm_engine.model_config.max_model_len == 32768
    ...
    def run(self,
        ...
        prompt = self.tokenizer.decode(self.tokenizer.encode(prompt, add_special_tokens=False)[:30768])
        messages = [
