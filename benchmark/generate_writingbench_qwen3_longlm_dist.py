...
# -*- coding: utf-8 -*-
"""
LongLM self‑extend inference script (multi‑GPU, multi‑process)

Key fixes vs. original:
1. **CUDA_VISIBLE_DEVICES is set _before_ importing any CUDA‑aware libs** in each worker, so every
   subprocess only sees the GPU(s) it should own.
2. After the import we pin the process to its unique visible GPU with `torch.cuda.set_device(0)`.
3. Heavy imports (`torch`, `transformers`, `LongLM.selfextend`) are moved inside the worker so the
   environment variable takes effect.
4. Added `device_map={"": 0}` to ensure `transformers` doesn’t shard the model across all visible
   GPUs (there is only one per worker anyway).
5. Tidied seeding utilities (local import of `torch`).

Tested with `world_size=8, size=1, shift=0` on an 8‑GPU machine – each rank was bound to a distinct
physical card (0‑7) without OOM.
"""
